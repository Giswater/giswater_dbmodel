...
AS WITH typevalue AS (
        ...
        UNION ALL
         ...
         SELECT DISTINCT ON (node_id) node.node_id,
            ...
            node.presszone_id,
            presszone_table.presszone_name,
           ...
           FROM node_selector
             ...
             LEFT JOIN presszone_table ON presszone_table.presszone_id::text = node.presszone_id::text
             ...
             LEFT JOIN man_valve m ON m.node_id::text = node.node_id::text
        )
 SELECT node_id,
    code,
    elevation,
        ...
        UNION ALL
         ...
         SELECT DISTINCT ON (arc.arc_id) arc.arc_id,
           ...
           FROM arc_selector
             ...
             LEFT JOIN presszone_table ON presszone_table.presszone_id::text = arc.presszone_id::text
...
AS WITH typevalue AS (
         ...
         SELECT dma.dma_id,
            ...
            dma.macrodma_id,
            dma.stylesheet,
            t.id::character varying::text AS dma_type
           FROM dma
             LEFT JOIN typevalue t ON t.id::text = dma.dma_type::text AND t.typevalue::text = 'dma_type'::text
        ), presszone_table AS (
         SELECT presszone.presszone_id,
         ...
         SELECT l.link_id,
            ...
            l.minsector_id,
            l.staticpressure,
            NULL::integer AS macrominsector_id,
            sector_table.sector_type,
            presszone_table.presszone_type,
            dma_table.dma_type,
            dqa_table.dqa_type
           FROM link l
             ...
             LEFT JOIN presszone_table ON presszone_table.presszone_id::text = l.presszone_id::text
         ...
         SELECT connec.connec_id,
           ...
           FROM connec
             ...
             LEFT JOIN ( SELECT connec_psector.connec_id,
                   ...
                   FROM connec_psector
                  WHERE connec_psector.p_state = 0) a USING (connec_id, arc_id)
         ...
         SELECT connec.connec_id,
            ...
            exploitation.macroexpl_id,
                ...
                CASE
                    ...
                    ELSE link_planned.presszone_id::character varying
                END AS presszone_id,
                CASE
                ...
                END AS dma_name,
                CASE
                    WHEN link_planned.dma_type IS NULL THEN dma_table.dma_type::character varying
           ...
           FROM inp_network_mode,
            connec_selector
             ...
             LEFT JOIN presszone_table ON presszone_table.presszone_id::text = connec.presszone_id::text
