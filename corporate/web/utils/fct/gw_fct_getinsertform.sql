...
--  get api version
    EXECUTE 'SELECT row_to_json(row) FROM (SELECT value FROM config_param_system WHERE parameter=''ApiVersion'') row'
        INTO api_version;
        RAISE NOTICE 'ID -> %',id;
        IF id IN ('NULL', '"NULL"', '"null"', '""', '''''') THEN
		-- fix diferent ways to say null on client
		id = REPLACE (id::text, '', null);
	END IF;
	RAISE NOTICE 'ID -> %',id;
...
--    Get fields
    EXECUTE 'SELECT array_agg(row_to_json(a)) FROM (SELECT label, name, type, "dataType", placeholder, widgetfunction as "widgetAction" FROM config_web_fields WHERE table_id = $1 order by orderby) a'
        INTO fields_array
        USING table_id;    
