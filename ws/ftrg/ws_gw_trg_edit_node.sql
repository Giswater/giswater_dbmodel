...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		-- FEATURE INSERT
		INSERT INTO node (node_id, code, elevation, depth, nodecat_id, epa_type, sector_id, arc_id, parent_id, state, state_type, annotation, observ,comment, dma_id, dqa_id, presszone_id,
		...
		NEW.comment,NEW.dma_id, NEW.dqa_id, NEW.presszone_id, NEW.soilcat_id, NEW.function_type, NEW.category_type, NEW.fluid_type, NEW.location_type,NEW.workcat_id, NEW.workcat_id_end, NEW.workcat_id_plan, NEW.buildercat_id,
		...
		NEW.om_state, NEW.conserv_state, NEW.access_type, NEW.placement_type, NEW.expl_id2, NEW.brand_id, NEW.model_id, NEW.serial_number, NEW.label_quadrant, NEW.streetname, NEW.streetname2);
	...
	-- static pressure
	  ...
	  IF (NEW.nodecat_id != OLD.nodecat_id) THEN
		...
		brand_id=NEW.brand_id, model_id=NEW.model_id, serial_number=NEW.serial_number, label_quadrant=NEW.label_quadrant, streetname = NEW.streetname, streetname2 = NEW.streetname2
