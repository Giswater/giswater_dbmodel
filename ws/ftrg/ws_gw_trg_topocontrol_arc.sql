...
BEGIN 
	...
	ELSIF (nodeRecord1.node_id IS NOT NULL) AND (nodeRecord2.node_id IS NULL) AND v_nodeinsert_arcendpoint THEN
		IF TG_OP = 'INSERT' THEN
			...
			-- Inserting new node
			INSERT INTO v_edit_node (sector_id, state, state_type, dma_id, presszone_id, soilcat_id, workcat_id, buildercat_id, builtdate, nodecat_id, ownercat_id, muni_id,
			postcode, district_id, expl_id, the_geom)
			VALUES (NEW.sector_id, NEW.state, NEW.state_type, NEW.dma_id, NEW.presszone_id, NEW.soilcat_id, NEW.workcat_id, NEW.buildercat_id, NEW.builtdate, v_nodecat,
