...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		INSERT INTO dma (dma_id, name, descript,  undelete, expl_id, pattern_id, link, effc, graphconfig, stylesheet, avg_press, dma_type)
		VALUES (NEW.dma_id, NEW.name, NEW.descript, NEW.undelete, NEW.expl_id, NEW.pattern_id, NEW.link,
		NEW.effc, NEW.graphconfig::json, NEW.stylesheet::json, NEW.avg_press, NEW.dma_type);

		IF view_name = 'ui' THEN
			UPDATE dma SET macrodma_id = (select macrodma_id from macrodma where name = new.macrodma), active = NEW.active WHERE dma_id = NEW.dma_id;
