	...
	IF TG_OP = 'INSERT' THEN
		...
		ELSE
			IF (SELECT true from cat_arc where id=NEW.arccat_id) IS NULL THEN
				EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
				"data":{"message":"3282", "function":"1302","debug_msg":"'||NEW.arccat_id||'"}}$$);';
			END IF;
		END IF;
		...
		-- FEATURE INSERT
		INSERT INTO arc (arc_id, code, arccat_id, epa_type, sector_id, "state", state_type, annotation, observ,"comment",custom_length,dma_id, dqa_id, presszone_id, soilcat_id, function_type, category_type, fluid_type, location_type,
					...
					VALUES (NEW.arc_id, NEW.code, NEW.arccat_id, NEW.epa_type, NEW.sector_id, NEW."state", NEW.state_type, NEW.annotation, NEW.observ, NEW.comment, NEW.custom_length,NEW.dma_id, NEW.dqa_id, NEW. presszone_id,
