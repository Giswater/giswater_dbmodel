...
BEGIN
	...
	IF (SELECT count(*) FROM selector_inp_dscenario WHERE cur_user = current_user) > 0 THEN
		...
		ELSIF v_patternmethod = 13 THEN -- DMA PATTERN (NODE)
			UPDATE temp_demand SET pattern_id=sector.pattern_id FROM node JOIN dma ON dma.dma_id=node.dma_id WHERE temp_demand.feature_id=node.node_id
			 AND temp_demand.pattern_id IS NULL;
			UPDATE temp_demand SET pattern_id=sector.pattern_id FROM connec JOIN dma ON dma.dma_id=connec.dma_id WHERE temp_demand.feature_id=connec.connec_id
			 AND temp_demand.pattern_id IS NULL;
