...
BEGIN
	...
	IF v_graphiclog THEN
		...
		FROM (SELECT node_id as id, fid, 'ERROR-228: Orphan node' as descript, the_geom FROM anl_node WHERE cur_user="current_user"() AND fid = 228
			...
			UNION
		      SELECT node_id, fid, 'ERROR-166: Node2arc with more than two arcs', the_geom FROM anl_node WHERE cur_user="current_user"() AND fid = 166		      
		      ) row
		      ) features;
		...
		FROM (
			...
			UNION
			SELECT arc_id as id, fid, 'ERROR-454: Arc with state=1 and without node_1 or node_2'::text as descript, the_geom FROM anl_arc WHERE cur_user="current_user"() AND fid=454
		     ) row) features;
