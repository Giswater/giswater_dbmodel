...
BEGIN
	...
	v_open = (select count(*) from temp_csv where source ='rpt_arc' and csv5 in ('OPEN', 'Open'));

	if v_open > 0 THEN

		DELETE FROM temp_csv WHERE csv1 = 'Page';
		
		-- forcing time values 
		for v_rec in select id, csv4 from temp_csv where csv2 like 'Results%' order by id desc
		loop 
			if v_time_last = 0 then v_time_last = v_rec.id;end if;
			raise notice ' v_rec % v_time_last %', v_rec, v_time_last;
			update temp_csv set csv40 = v_rec.csv4 where id > v_rec.id and  id < v_time_last;
		...
		DELETE FROM temp_csv where csv2 in ('CMH', 'LPS', 'LPM', 'GPM', 'AFD', 'CMD', 'ALD', 'IMGD', 'MGD');
		...
		DELETE FROM temp_csv WHERE source='rpt_node' AND (csv3='m' or csv5 = 'Hours');
