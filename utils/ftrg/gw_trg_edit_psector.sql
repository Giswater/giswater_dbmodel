...
BEGIN
	...
	IF om_aux='om' THEN
		...
		sector_id, atlas_id, gexpenses, vat, other, the_geom, expl_id, active, status, ext_code)
	...
	ELSIF om_aux='plan' THEN
		...
		INSERT INTO plan_psector (psector_id, name, psector_type, descript, priority, text1, text2, observ, rotation, scale, sector_id,
		 atlas_id, gexpenses, vat, other, the_geom, expl_id, active, status, ext_code)
    ...
    ELSIF TG_OP = 'UPDATE' THEN
		...
		IF (OLD.status != NEW.status) AND (NEW.status = 0 OR NEW.status = 2 OR NEW.status = 3)  THEN
			...
			FOR rec_type IN (SELECT * FROM sys_feature_type WHERE id IN ('NODE', 'ARC') ORDER BY id asc) LOOP
