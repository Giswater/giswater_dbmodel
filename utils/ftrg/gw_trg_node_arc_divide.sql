...
--  Only enabled on insert
	IF TG_OP = 'INSERT' AND edit_arc_division_dsbl_aux IS NOT TRUE THEN
		...
		SELECT node_id INTO node_id_aux FROM v_edit_node WHERE ST_intersects((NEW.the_geom), St_buffer(v_edit_node.the_geom,v_node_proximity)) AND NEW.state>0 AND node_id != NEW.node_id LIMIT 1;
