...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		VALUES (NEW.id, NEW.distance, NEW.depth, NEW.the_geom, NEW.x_label, NEW.y_label, NEW.rotation_label, NEW.offset_label, NEW.direction_arrow, NEW.x_symbol, NEW.y_symbol, 
			NEW.feature_id, NEW.feature_type, NEW.state, NEW.expl_id, NEW.observ, NEW.comment);
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		UPDATE dimensions
		SET id=NEW.id, distance=NEW.distance, depth=NEW.depth, the_geom=NEW.the_geom, x_label=NEW.x_label, y_label=NEW.y_label, rotation_label=NEW.rotation_label, 
		offset_label=NEW.offset_label, direction_arrow=NEW.direction_arrow, x_symbol=NEW.x_symbol, y_symbol=NEW.y_symbol, feature_id=NEW.feature_id, 
		feature_type=NEW.feature_type, expl_id=NEW.expl_id, observ=NEW.observ, comment=NEW.comment, state=NEW.state
		WHERE id=NEW.id;
