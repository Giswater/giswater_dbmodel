...
BEGIN
   ...
   IF TG_OP='INSERT' THEN
		...
		ELSIF v_triggerfromtable ='om_visit_x_feature' THEN -- change feature_x_lot status (when function is triggered by om_visit_x_*
			...
			-- set class_id = visitcat_id for new visit coming from Add Visit plugin tool
			SELECT visitcat_id INTO v_visitcat FROM om_visit WHERE id=NEW.visit_id;

			IF v_visitcat IS NOT NULL THEN
				UPDATE om_visit SET class_id=v_visitcat WHERE id=NEW.visit_id;
			END IF;
