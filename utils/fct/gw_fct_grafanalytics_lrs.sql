...
BEGIN
	...
	FROM v_edit_node WHERE v_edit_node.node_id::text IN ('||v_node_string||') AND node_id NOT IN (select node_id FROM anl_node WHERE fprocesscat_id = '||v_fprocesscat_id||');';
