...
BEGIN
   ...
   if v_feature_type = 'connec' then
		...
		''',v_feature_type,'at_id'', ',v_feature_type,'at_id
		) AS combined_data
    ...
    LOOP
        ...
        IF (aux_json->>'columnname') = 'featurecat_id' then
        	if lower(v_project_type) = 'ws' then
            	v_sql := concat('SELECT array_agg(id) FROM cat_', v_feature_type, ' WHERE ', v_feature_type, 'type_id = ''', v_curr_featurecat, ''' AND active IS TRUE OR active IS NULL ORDER BY 1');
            else
        ...
        IF (aux_json->>'columnname') = 'fluid_type' THEN
            ...
            WHERE lower(feature_type) = v_feature_type
                AND (v_new_featurecat = any (featurecat_id::text[]) OR featurecat_id IS NULL)
