    ...
    ELSIF TG_OP = 'UPDATE' THEN
		...
		IF (NEW.field_checked is TRUE) THEN
			...
			IF EXISTS (SELECT gully_id FROM review_audit_gully WHERE gully_id=NEW.gully_id) THEN					
				...
				review_status_id=review_status_aux, field_date=now(), field_user=current_user
       			WHERE gully_id=NEW.gully_id;
			...
			ELSE
				...
				rec_gully.observ, NEW.observ, NEW.expl_id, NEW.the_geom, review_status_aux, now(), current_user);
