...
BEGIN
	...
	SELECT ((value::json)->>'activated')::boolean INTO v_doublegeometry FROM config_param_system WHERE parameter='insert_double_geometry';
	...
	IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') AND v_doublegeometry AND v_man_table = 'man_netgully' THEN
		...
		IF v_length*v_width IS NULL OR v_length*v_width = 0 THEN
			v_length = 500;
			v_width = 500;
		END IF;
	...
	-- Dma ID
		...
		ELSE
				...
				NEW.muni_id, NEW.streetaxis_id, NEW.postcode,NEW.streetaxis2_id,NEW.postnumber,NEW.postnumber2, NEW.postcomplement, NEW.postcomplement2,
				NEW.descript, NEW.rotation,NEW.link, NEW.verified, NEW.undelete, NEW.label_x,NEW.label_y,NEW.label_rotation,NEW.the_geom,
				NEW.expl_id, NEW.publish, NEW.inventory, NEW.uncertain, NEW.xyz_date, NEW.unconnected, NEW.num_value, now(), current_user, NEW.cat_matcat_id);	
		END IF;
		...
		ELSIF v_man_table='man_storage' THEN
			...
			IF (v_doublegeometry IS TRUE) THEN
				...
				INSERT INTO polygon(pol_id, sys_type, the_geom) VALUES (NEW.pol_id, 'STORAGE', (SELECT ST_Multi(ST_Envelope(ST_Buffer(node.the_geom,v_double_geom_buffer))) 
		...
		ELSIF v_man_table='man_netgully' THEN
			...
			IF (v_doublegeometry IS TRUE) THEN
				...
				INSERT INTO polygon(pol_id, sys_type, the_geom) VALUES (NEW.pol_id, 'NETGULLY', v_the_geom_pol);
			END IF;
		...
		ELSIF v_man_table='man_chamber' THEN
			...
			IF (v_doublegeometry IS TRUE) THEN
				...
				INSERT INTO polygon(pol_id, sys_type, the_geom) VALUES (NEW.pol_id,'CHAMBER', (SELECT ST_Multi(ST_Envelope(ST_Buffer(node.the_geom,v_double_geom_buffer))) from node where node_id=NEW.node_id));
		...
		ELSIF v_man_table='man_wwtp' THEN
			...
			IF (v_doublegeometry IS TRUE) THEN
				...
				INSERT INTO polygon(pol_id, sys_type, the_geom) VALUES (NEW.pol_id, 'WWTP', (SELECT ST_Multi(ST_Envelope(ST_Buffer(node.the_geom,v_double_geom_buffer)))
				from node where node_id=NEW.node_id));
		...
		ELSIF v_man_table='parent' THEN
			...
			IF (v_man_table IN ('man_chamber', 'man_storage', 'man_wwtp','man_netgully') 
				...
				VALUES (v_auto_pol_id, v_sys_type, (SELECT ST_Multi(ST_Envelope(ST_Buffer(node.the_geom,v_double_geom_buffer))) from node where node_id=NEW.node_id));
    ...
    -- UPDATE management values
		...
		ELSE
			...
			streetaxis2_id=NEW.streetaxis2_id, postnumber=NEW.postnumber, postnumber2=NEW.postnumber2, descript=NEW.descript, link=NEW.link, verified=NEW.verified, undelete=NEW.undelete, 
			label_x=NEW.label_x, label_y=NEW.label_y, label_rotation=NEW.label_rotation, publish=NEW.publish, inventory=NEW.inventory, rotation=NEW.rotation, uncertain=NEW.uncertain,
			xyz_date=NEW.xyz_date, unconnected=NEW.unconnected, expl_id=NEW.expl_id, num_value=NEW.num_value, lastupdate=now(), lastupdate_user=current_user, matcat_id = NEW.cat_matcat_id
			WHERE node_id = OLD.node_id;
		END IF;
