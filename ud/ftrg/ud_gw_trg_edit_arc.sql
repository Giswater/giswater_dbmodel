...
BEGIN
	...
	IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
		...
		IF NEW.streetname IS NOT NULL AND ((NEW.streetname NOT IN (SELECT DISTINCT descript FROM v_ext_streetaxis)) OR (NEW.streetname2 NOT IN (SELECT DISTINCT descript FROM v_ext_streetaxis))) THEN
			...
			"data":{"message":"3246", "function":"1202","debug_msg":null}}$$);';
		END IF;
	...
	IF TG_OP = 'INSERT' THEN
		...
		IF v_matfromcat THEN
			...
			drainzone_id, parent_id, expl_id2, adate, adescript, visitability, label_quadrant, brand_id, model_id, serial_number, streetname, streetname2)
			...
			NEW.drainzone_id, NEW.parent_id, NEW.expl_id2, NEW.adate, NEW.adescript, NEW.visitability, NEW.label_quadrant, NEW.brand_id, NEW.model_id, NEW.serial_number, NEW.streetname, NEW.streetname2);
		ELSE
			...
			NEW.asset_id, NEW.pavcat_id, NEW.drainzone_id, NEW.parent_id, NEW.expl_id2, NEW.adate, NEW.adescript, NEW.visitability, NEW.label_quadrant, NEW.brand_id, NEW.model_id, NEW.serial_number, NEW.streetname, NEW.streetname2);
		END IF;
		...
		IF v_man_table='man_conduit' THEN
			...
			INSERT INTO man_conduit VALUES (NEW.arc_id, NEW.inlet_offset, NEW.bottom_mat);
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		IF v_man_table='man_conduit' THEN
			...
			UPDATE man_conduit SET arc_id=NEW.arc_id, inlet_offset = NEW.inlet_offset, bottom_mat=NEW.bottom_mat WHERE arc_id=OLD.arc_id;
