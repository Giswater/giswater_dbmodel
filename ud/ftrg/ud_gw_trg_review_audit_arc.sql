	...
	IF TG_OP = 'UPDATE' THEN
		...
		ELSIF NEW.is_validated = 1 THEN
			...
			IF review_status=1 AND NEW.arc_id NOT IN (SELECT arc_id FROM arc) THEN 
				...
				INSERT INTO v_edit_arc (arc_id, y1, y2, arc_type, arccat_id, annotation, observ, expl_id, the_geom, cat_matcat_id)
				VALUES (NEW.arc_id, NEW.new_y1, NEW.new_y2, NEW.new_arc_type, NEW.new_arccat_id, NEW.new_annotation, NEW.new_observ, NEW.expl_id, NEW.the_geom, NEW.matcat_id); 
			...
			ELSIF review_status=2 THEN
				...
				observ=NEW.new_observ, arc_type=NEW.new_arc_type, cat_matcat_id=NEW.matcat_id
