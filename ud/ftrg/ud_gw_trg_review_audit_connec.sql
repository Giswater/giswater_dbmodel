	...
	IF TG_OP = 'UPDATE' THEN
		...
		ELSIF NEW.is_validated = 1 THEN
			...
			IF review_status=1 AND NEW.connec_id NOT IN (SELECT connec_id FROM connec) THEN 
				...
				INSERT INTO v_edit_connec (connec_id, y1, y2, connec_type, connecat_id, annotation, observ, expl_id, the_geom, cat_matcat_id)
				VALUES (NEW.connec_id, NEW.new_y1, NEW.new_y2, NEW.new_connec_type, NEW.new_connecat_id, NEW.annotation, NEW.observ, NEW.expl_id, NEW.the_geom, NEW.new_matcat_id); 
			...
			ELSIF review_status=2 THEN
				...
				connec_type=NEW.new_connec_type, annotation=NEW.new_annotation, observ=NEW.new_observ, cat_matcat_id=NEW.new_matcat_id
			...
			ELSIF review_status=3 THEN
				...
				annotation=NEW.new_annotation, observ=NEW.new_observ, cat_matcat_id=NEW.new_matcat_id
				WHERE connec_id=NEW.connec_id;
