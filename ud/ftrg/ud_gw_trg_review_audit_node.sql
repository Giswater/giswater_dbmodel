	...
	IF TG_OP = 'UPDATE' THEN
		...
		ELSIF NEW.is_validated = 1 THEN
			...
			IF review_status=1 AND NEW.node_id NOT IN (SELECT node_id FROM node) THEN 
				INSERT INTO v_edit_node (node_id, top_elev, ymax, node_type, nodecat_id, annotation, observ, expl_id, the_geom, cat_matcat_id)
			...
			ELSIF review_status=2 THEN
				...
				node_type=NEW.new_node_type, annotation=NEW.new_annotation, observ=NEW.new_observ, cat_matcat_id=NEW.new_matcat_id
