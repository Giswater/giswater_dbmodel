	...
	IF TG_OP = 'UPDATE' THEN
		...
		ELSIF NEW.is_validated = 1 THEN
			...
			IF v_review_status=1 AND NEW.gully_id NOT IN (SELECT gully_id FROM gully) THEN 
				...
				INSERT INTO v_edit_gully (gully_id, top_elev, ymax, sandbox, gratecat_id, gully_type, units, groove, siphon,  connec_arccat_id, annotation, observ, expl_id, the_geom)
			...
			ELSIF v_review_status=2 THEN
				...
				siphon=NEW.new_siphon, connec_arccat_id=NEW.new_connec_arccat_id, annotation=NEW.new_annotation, observ=NEW.new_observ WHERE gully_id=NEW.gully_id;
