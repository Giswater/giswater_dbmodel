...
ORDER:
timseries, timser_type, times_type, idval, expl_id, date, hour, time, value
csv1, csv2, csv3, csv4, csv5, csv6, csv7, csv8, csv9
csv1 = timseries, csv2 = timser_type, csv3 = times_type, csv4 = idval, csv5 = expl_id, csv6 = date, csv7 = hour, csv8 = time, csv9 = value
	...
	LOOP
		IF rec_csv.csv1 IS NOT NULL THEN -- to control those null rows because user has a bad structured csv file (common last lines)
			...
			IF rec_csv.csv1 NOT IN (SELECT id FROM inp_timeseries) THEN
				...
				INSERT INTO inp_timeseries (id, timser_type, times_type, idval, expl_id, log, addparam)
