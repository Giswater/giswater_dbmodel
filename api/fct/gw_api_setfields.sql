...
--  get api version
	...
	IF v_type = 'VALVE' AND v_closedstatus IS NOT NULL THEN
		...
		IF v_automaticmapzonetrigger THEN
			...
			-- FOR v_mapzone
			FOR v_mapzone IN 
			SELECT upper(mapzone) FROM (SELECT 'sector' AS mapzone, value::json->>'SECTOR' as status FROM config_param_system WHERE parameter='om_dynamicmapzones_status'
			UNION
			SELECT 'presszone', value::json->>'PRESSZONE' FROM config_param_system WHERE parameter='om_dynamicmapzones_status'
			UNION
			SELECT 'dma', value::json->>'DMA' FROM config_param_system WHERE parameter='om_dynamicmapzones_status'
			UNION
			SELECT 'dqa', value::json->>'DQA' FROM config_param_system WHERE parameter='om_dynamicmapzones_status') a
			...
			LOOP
				...
				IF v_closedstatus IS TRUE OR (v_closedstatus IS FALSE AND v_count = 2) THEN
					...
					-- getting nodeheader 
					IF v_mapzone = 'presszonecat' THEN
						SELECT (json_array_elements_text((grafconfig->>'use')::json))::json->>'nodeParent'INTO v_nodeheader FROM cat_presszone WHERE id::integer = v_mapzone_id;
						v_mapzone = 'PRESSZONE';	
					ELSE
	...
	-- Return
	RETURN ('{"status":"Accepted", "message":'||v_message||', "version":' || api_version ||
