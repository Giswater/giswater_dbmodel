...
/*example
CURRENT
SELECT SCHEMA_NAME.gw_api_getselectors($${"client":{"device":9, "infoType":100, "lang":"ES"}, "form":{}, "feature":{}, "data":{"filterFields":{}, "pageInfo":{}, 
"selector_type":{"mincut": {"ids":[1, 10, 12, 13, 14, 15, 16, 17], "table":"anl_mincut_result_selector", "view":"anl_mincut_result_cat"}}}}$$)::text

PROPOSED
SELECT SCHEMA_NAME.gw_api_getselectors($${"client":{"device":9, "infoType":100, "lang":"ES"}, "form":{}, "feature":{}, "data":{"filterFields":{}, "pageInfo":{}, "selectorType":"mincut"}}$$)::text
*/
...
--	Variables
	...
	v_label_table text;
	v_label_selector text;
	v_table text;
	v_selector text;
	v_selector_type_ text;
...
-- Start the construction of the tabs array
	...
	SELECT json_object_keys(v_selector_type) INTO v_selector_type_;
	...
	IF rec_tab.id IS NOT NULL THEN
		...
		v_parameter_selector = (SELECT value::json FROM config_param_system WHERE parameter = 'api_selector_' || v_selector_type_);
		v_label_table = v_parameter_selector->>'label_table';
		v_label_selector = v_parameter_selector->>'label_selector';
		v_table = v_parameter_selector->>'table';
		v_selector = v_parameter_selector->>'selector';
		

		-- Manage selectors list
		IF v_selectors_list = '()' OR v_selectors_list IS NULL THEN
			v_selectors_list = '(-1)';
		END IF;
		
		-- Get exploitations, selected and unselected with selectors list
		EXECUTE 'SELECT array_to_json(array_agg(row_to_json(a))) FROM (
		SELECT concat(' || v_label_table || ') AS label, id::text as widgetname, ''result_id'' as column_id, ''check'' as type, ''boolean'' as "dataType", true as "value" 
		...
		UNION
		SELECT concat(' || v_label_table || ') AS label, id::text as widgetname, ''result_id'' as column_id, ''check'' as type, ''boolean'' as "dataType", false as "value" 
